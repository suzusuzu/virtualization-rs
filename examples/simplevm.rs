...
fn main() {
    ...
    unsafe {
        ...
        let boot_loader = VZLinuxBootLoaderBuilder::new()
            .kernel_url(canonicalize(&kernel).unwrap().into_os_string().into_string().unwrap())
            .initial_ramdisk_url(canonicalize(&initrd).unwrap().into_os_string().into_string().unwrap())
        ...
        match conf.validate_with_error() {
            Ok(_) => {
                ...
                let queue = dispatch_queue_create(label.as_ptr(), NIL);
                ...
                let dispatch_block = ConcreteBlock::new(move || {
                    let completion_handler = ConcreteBlock::new(|err: Id| {
                        if err != NIL {
                            let error = NSError(StrongPtr::new(err));
                            error.dump();
                        }
                    });
